...
import { Clock, ChevronRight, Timer, CheckCircle } from "lucide-react";
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-4 py-4">
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-6 justify-center px-2">
          {questions.map((q, idx) => {
            ...
            let colorClass =
              "bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 text-slate-400 dark:text-slate-500"; // Default
            ...
            if (ans) {
              ...
              colorClass =
                "ring-2 ring-blue-500 border-blue-500 text-blue-600 dark:text-blue-400 font-bold";
            }
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="space-y-2 sm:space-y-3">
              {(["A", "B", "C", "D"] as const).map((optionKey) => {
                ...
                let buttonStyle =
                  "border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-500 hover:bg-slate-50 dark:hover:bg-slate-700 active:scale-98"; // Default
                let badgeStyle =
                ...
                return (
                  ...
                  >
                    <span
                      className={`flex-shrink-0 w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-xs sm:text-sm font-bold border ${badgeStyle}`}
                    >
                      {optionKey}
                    </span>
                    <span
                      className={`${textStyle} text-sm sm:text-base leading-relaxed pt-0.5`}
                    >
        ...
        <div className="max-w-3xl mx-auto flex justify-between items-center gap-2">
          ...
          {currentIndex === questions.length - 1 ? (
            <button
              ...
              className="bg-green-600 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg font-bold shadow-lg hover:bg-green-700 transition-all text-sm sm:text-base active:scale-95"
            >
              {t("quiz_finish")}
            </button>
          ) : (
